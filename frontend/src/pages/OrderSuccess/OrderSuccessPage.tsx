...
export const OrderSuccessPage: React.FC = () => {
  ...
  return (
    <>
      <div className="screen-only max-w-4xl mx-auto px-4 py-12 sm:py-16 text-left space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
          <Link to="/products">
            <Button variant="gold" size="md" rightIcon={<ArrowRight className="w-4 h-4 shrink-0" />}>
              CONTINUE SHOPPING
            </Button>
          </Link>
          ...
          <Button
            ...
            leftIcon={<MessageCircle className="w-4 h-4 shrink-0" />}
          >
          ...
          {order && (
            <Button
              ...
              leftIcon={<FileText className="w-4 h-4 shrink-0 text-[#D4AF37]" />}
            >
              PREVIEW INVOICE
            </Button>
          )}
      ...
      {/* 5. INTERACTIVE ON-SCREEN INVOICE MODAL PREVIEW */}
      {showInvoicePreview && order && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 overflow-y-auto no-print">
          <div className="bg-[#181821] border border-[#3A3A4A] rounded-xl shadow-2xl max-w-4xl w-full max-h-[92vh] flex flex-col overflow-hidden text-white my-auto">
            {/* Modal Header */}
            <div className="px-5 py-3.5 border-b border-[#2A2A36] flex items-center justify-between bg-[#121216]">
              <div className="flex items-center gap-2 font-sport">
                <FileText className="w-5 h-5 text-[#D4AF37]" />
                <span className="font-black text-sm uppercase tracking-wider text-[#F4F4F5]">
                  OFFICIAL TAX INVOICE PREVIEW
                </span>
                <span className="text-xs text-[#71717A]">#{order.order_number}</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 rounded-md text-[#A1A1AA] hover:text-white hover:bg-[#24242D] transition-colors cursor-pointer"
                  title="Close"
                >
                  <X className="w-5 h-5" />
                </button>
            ...
            {/* Modal Body - Paper Preview Container */}
            <div className="flex-1 overflow-y-auto p-3 sm:p-6 bg-neutral-900/90 flex justify-center">
              <div className="shadow-2xl border border-neutral-300 rounded-xs overflow-hidden w-full max-w-[800px]">
                <ProfessionalInvoice order={order} />
              </div>
            </div>

            {/* Modal Footer */}
            <div className="px-5 py-3 border-t border-[#2A2A36] bg-[#121216] flex items-center justify-between text-xs font-sport text-[#71717A]">
              <span>Single-page A4 certified tax invoice format</span>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setShowInvoicePreview(false)}
      ...
      {/* 6. DEDICATED PRINT-ONLY SINGLE PAGE INVOICE */}
      {order && (
